    ...
    def command(self):
        ...
        if self.commandList[0] in ["get", "take", "obtain", "grab", "acquire"]:
            if self.commandList[1] in self.map.inventory[self.location]:
                 ...
                 if "goblin" in self.inventory or "bloodyape" in self.inventory or "nightmare" in self.inventory:
        ...
        if self.commandList[0] in ["use","drink"]:
            if self.commandList[1] == "healthpotion" and "healthpotion" in self.inventory:
...
if P.alive == False:
    ...
    print("Baron pays his effort and finally wins his round! ")