# -*- coding: utf-8 -*-
"""
Created on Fri Mar 23 13:18:12 2018
...
class map:
    def __init__(self):
        ...
        self.inventory = [[],["rusty crowbar"],["health potion","locked door"],["nightmare"],["key"],[],[],["shovel","nightmare"],["health potion"],["master sword","keepsake ring"],["bloody ape"],[],[],[]]
...
class player:
    ...
    def move(self):
        ...
        try:
            if self.map.self.direction[self.location][self.direction] != -1:
                print("You have moved to "+self.map.roomNames[self.map.self.direction[self.location][self.direction]])
                self.location = self.map.self.direction[self.location][self.direction]
            else:
        ...
        if self.location == 7 and "nightmare" in self.map.inventory[self.location]:
            ...
            return
                #Try to move to a new room; set the player's new location accordingly
        try:
            if self.map.direction[self.location][direction] != -1:
                print("You have moved to "+self.map.roomNames[self.map.direction[self.location][direction]])
                self.location = self.map.direction[self.location][direction]
            else:
                print("Where are you going to? You cannot move in that direction.")
        #Indicate if the desired direction is not available
        except:
    ...
    def showstatus(self):
        ...
        print("---------------------")
        print("You are in the "+self.roomNames.map[self.location])
        print("---------------------")
    ...
    def health(self):
        ...
        if self.Bn == ("fight","attack", "slice", "hack", "destroy"):
            ...
            self.Bahp-250
        if self.Nmhp == 0:
            print("Baron defeated the nightmare!!")
        if self.Bahp == 0:
            print("Baron defeated the bloody ape!!")
