...
rooms = {
    ...
    def checkVictory(self):#define wheter player could beat the monster
        if self.location == 10 and "master sword" , "keepsake ring" in self.inventory:
            self.victory = True
    
    def showRoomInventory(self):#define which items players could see in the room
        print("here is the following:")
        for i in self.map.inventory[self.location]:
            print (i)
            
    def showInventory(self):#show the item players already got
        print("Now you have the following:")
        for i in self.inventory:
            print(i)
            
    def showDirections(self):#give choice let players move 
