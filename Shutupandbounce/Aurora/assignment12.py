...
class Recursive():
    def Factorials(self,a):
        if self.a == 1 :
            ...
            self.fa = self.a * factorial(self.a - 1)  
            return self.fa
    def Hanoi(self,a,A,B,C): 
        if (self.a == 1) :
            self.move(self.A, self.C)   #when there is only one plate,move it from A pillar to C pillar
        else :
            ...
            self.move(self.A, self.C)              #move the last plate on A pillar directly to the C pillar
            self.Hanoi(self.n - 1, self.B, self.A, self.C) 
