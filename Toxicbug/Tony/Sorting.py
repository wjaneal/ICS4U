...
class sorting:
    ...
    def excute(self):#Mix them together to excute it
        ...
        self.time2 = []
        self.time3 = []
        self.n = [1000,2000,3000,4000,5000]#It shows the range of the numbers
        for i in self.n:
    ...
    def graph(self):#graph the range of the number and the time
        pl.plot(self.n,self.time1)
        pl.plot(self.n,self.time2)
        pl.plot(self.n,self.time3)
        pl.xlable('Range')
        pl.ylabel('Time')
        pl.show()
H = sorting()
